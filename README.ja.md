...
- **サービス稼働ランプ** — バナーの記号で Claude Code の可用性を一目で表示 (status.claude.com)
...
1 行目の先頭記号は [status.claude.com](https://status.claude.com) の **Claude Code** コンポーネントの信号機を兼ねます。ステータスは分離したバックグラウンドプロセスで取得され 5 分間キャッシュされます。描画をブロックすることはありません。正常時と不明時はフローレット (`✻`) のままで異常時は形と色の両方が変化します。
