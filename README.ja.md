...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 用カスタムステータスライン — モデル情報、コンテキスト使用量グラデーションバー、トークン統計、コスト、Git ステータス、使用量制限を表示します。

![Demo](assets/claude-code-statusline-demo.png)

## Claude Code 内での表示

![Claude Code 内での表示](assets/claude-code-statusline-overview.png)
...
Claude Code プラグインが推奨インストール方法です。npm は代替手段です。どちらも同じ `statusLine` 設定を `~/.claude/settings.json` に書き込みます。
...
| 行  | 内容                                                                                             |
| --- | ------------------------------------------------------------------------------------------------ |
| 1   | ステータスランプ、Claude Code と Statusline の両バージョン、各チェック有効時に `→ vX.Y.Z` を併記 |
...
1 行目の先頭記号は [status.claude.com](https://status.claude.com) の **Claude Code** コンポーネントの信号機を兼ねます。ステータスは分離したバックグラウンドプロセスで取得され 5 分間キャッシュされます。描画をブロックすることはありません。正常時と不明時はフローレット (`✻`) のままで異常時は形と色の両方が変化します。記号は OSC 8 ハイパーリンクにもなっており、Cmd + クリック (macOS) または Ctrl + クリックでステータスページを開けます。

| 記号 | 色       | サービス状態         | 意味                        |
| ---- | -------- | -------------------- | --------------------------- |
| `✻`  | グリーン | Operational          | すべて正常 — 安心して使用可 |
| `◐`  | ゴールド | Degraded Performance | 通常より遅い                |
| `▲`  | コーラル | Partial Outage       | 一部機能に影響              |
| `■`  | レッド   | Under Maintenance    | 計画メンテナンス中          |
| `✘`  | レッド   | Major Outage         | サービス停止                |
| `✻`  | グレー   | Unknown              | 未チェックまたは不明な状態  |
...
Claude Code は各レンダリングサイクルで stdin を通じて `statusLine` コマンドに JSON オブジェクトを送信します。JSON には現在のセッション状態（モデル、トークン、コスト、ワークスペース、使用量制限など）が含まれます。本ツールはそれを解析し ANSI カラーテキストを stdout に出力します。
