...
function reserveCheck({ target, current, home = os.homedir() } = {}) {
  const prev = readCache(home, target);
  return writeCache(
    {
      checkedAt: Date.now(),
      current,
...
function peekUpdate({
  ...
  if (
    isStale(cache) &&
    reserveCheck({ target, current: currentVersion, home })
  ) {
...
function resolveTargets(target) {
  if (target === "all") return TARGET_NAMES;
  if (TARGET_NAMES.includes(target)) return [target];
  throw new Error(
    `Invalid target "${target}". Expected one of: ${[...TARGET_NAMES, "all"].join(", ")}.`,
  );
}
...
function run(args, { home = os.homedir() } = {}) {
  try {
    ...
    setUpdateCheck(target, value, { home });
    console.log(`Set ${target} update check to ${value}`);
