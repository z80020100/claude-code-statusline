...
function render(data, options = {}) {
  ...
  const osc8Re = /\x1b\]8;;[^\x1b\x07]*(?:\x1b\\|\x07)/g;
  function visibleLen(s) {
    return s.replace(osc8Re, "").replace(ansiRe, "").length;
  }

  // OSC 8 hyperlink: clickable in iTerm2 / Kitty / WezTerm; inert elsewhere.
  const osc8Link = (url, text) => `\x1b]8;;${url}\x1b\\${text}\x1b]8;;\x1b\\`;
  ...
  const sandboxStates = {
    auto: `${gray}Sandbox${reset} ${green}${icons.shield}${icons.lightning} auto${reset}`,
    on: `${gray}Sandbox${reset} ${teal}${icons.shield} on${reset}`,
    "": `${gray}Sandbox${reset} ${dim}${icons.shield} off${reset}`,
  };
  const sandboxStr = sandboxStates[sandboxMode];

  // Status lamp: the leading glyph doubles as a usability traffic-light so a
  // glance answers "can I use Claude Code right now?". operational and unknown
  // keep the florette (green / gray); abnormal states swap both shape and color.
  // Every glyph is a single-column code point with no emoji-variation selector,
  // so the banner width stays stable. Colors reuse the existing status spectrum.
  function statusLamp(status) {
    const lamps = {
      operational: { glyph: "✻", color: green }, // ✻ usable
      degraded_performance: { glyph: "◐", color: gold }, // ◐ slow
      partial_outage: { glyph: "▲", color: coral }, // ▲ partial impact
      under_maintenance: { glyph: "■", color: red }, // ■ stopped (planned)
      major_outage: { glyph: "✘", color: red }, // ✘ down
    };
    return lamps[status] ?? { glyph: "✻", color: gray }; // unknown / future
  }

  const shortId = sessionId ? sessionId.slice(0, 7) : "";
  const sessionStr = shortId
    ? sessionName
      ? `${softWhite}${sessionName}${reset} ${gray}(${shortId})${reset}`
      : `${gray}(${shortId})${reset}`
    : "";
  ...
  if (version) {
    ...
    const arrow = (p) =>
      p.available ? ` ${gold}\u2192 v${p.latest}${reset}` : "";

    claudeUpdateSuffix = arrow(peek("claude", version));
    statuslineSegment =
      `${dim}${claudeOrange}Statusline${reset} ${dim}v${PKG.version}${reset}` +
      arrow(peek("statusline", PKG.version));

    // Live component status drives the leading lamp glyph; peek is non-blocking.
    const liveStatus =
      options.claudeStatus !== undefined
        ? options.claudeStatus
        : claudeStatus.peekStatus({ home });
    lamp = statusLamp(liveStatus?.status);
  }

  // · joins Claude Code and Statusline as a list rather than a partition.
  const versionLine = version
    ? `${lamp.color}${osc8Link("https://status.claude.com/", lamp.glyph)}${reset} ${claudeOrange}${bold}Claude Code${reset} ${dim}v${version}${reset}${claudeUpdateSuffix} ${darkGray}\u00B7${reset} ${statuslineSegment}`
    : "";

  const envParts = [sandboxStr];
  if (sessionStr) envParts.push(sessionStr);
  const envLine = envParts.join(sep);
